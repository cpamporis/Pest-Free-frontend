// apiService.js - Pestify Security Lab only
...
if (
  !API_BASE_URL.startsWith(SECURITY_LAB_API_ORIGIN) ||
  API_BASE_URL.includes("field-inspections-backend-production")
) {
...
// Load token once before any request or mutation touches auth state.
const authStorageReady = (async () => {
  try {
    ...
    authToken = null;
    console.error("Failed to load the stored authentication token");
  }
})();
...
async function clearAuthToken() {
  ...
  try {
    ...
    console.error("Failed to clear the stored authentication token");
  }
}
